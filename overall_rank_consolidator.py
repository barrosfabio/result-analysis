...
path = "C:\\Users\\Fabio Barros\\Desktop\\IJCNN Novos Resultados\\Rydls + C19 IDC\\overall_analysis"
...
def build_final_df_classifiers(path, results_df):
    ...
    final_results_df['resampling'] = results_df.iloc[:, 12]
    final_results_df.to_csv(path, sep=';')
    return final_results_df
...
for analysis in analysis_array:
    ...
    algorithm_list = os.listdir(per_class_analysis)
    for algorithm in algorithm_list:
        file_name = 'avg_ranks_' + algorithm + '.csv'
        file_path = per_class_analysis + '\\' +algorithm + '\\' + file_name
